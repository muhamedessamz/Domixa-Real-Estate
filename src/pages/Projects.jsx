        ...
        <div className="pt-40 pb-32 bg-white min-h-screen">
                    ...
                    {projects.map((project, idx) => (
                        ...
                        >
                            ...
                            <div className="flex-1 w-full relative group">
                                ...
                                <div className="absolute -bottom-6 md:-bottom-10 right-6 md:right-12 rtl:left-6 rtl:md:left-12 rtl:right-auto bg-white p-4 md:p-8 shadow-2xl rounded-2xl md:rounded-3xl flex items-center gap-4 md:gap-6 border border-gray-50 transform group-hover:translate-y-[-10px] transition-transform duration-500">
                                    <div className="bg-domixa-gold/10 p-3 md:p-4 rounded-xl md:rounded-2xl text-domixa-gold">
                                        <Calendar className="w-5 h-5 md:w-6 md:h-6" />
                                    </div>
                                    <div>
                                        <p className="text-[8px] md:text-[10px] uppercase font-black text-gray-300 tracking-[0.2em] mb-0.5 md:mb-1">{t('project.feed.status')}</p>
                                        <p className="text-xs md:text-sm font-black text-domixa-dark uppercase">{project.status[i18n.language]}</p>
                                    </div>
                                </div>
                            </div>

                            {/* Text Area */}
                            <div className="flex-1 space-y-10 flex flex-col items-center md:items-start text-center md:text-left rtl:md:text-right">
                                ...
                                <h2 className={`text-4xl md:text-6xl ${i18n.language === 'ar' ? 'font-arabic-title font-black leading-[1.2]' : 'font-serif font-black italic leading-tight'} text-domixa-dark`}>
                                ...
                                <div className="pt-4 flex flex-col sm:flex-row items-center gap-8">
                                    <button className="btn-elite rounded-2xl flex items-center gap-3 w-full sm:w-auto justify-center">
            ...
            <section className="mt-40 section-premium bg-domixa-gray">
                <div className="max-w-[1400px] mx-auto text-center">
                    ...
                    <div className="flex flex-wrap lg:grid lg:grid-cols-5 gap-12 lg:gap-20 items-center justify-center">
                        {developers.map((dev) => (
                            <div key={dev.id} className="w-[calc(50%-1.5rem)] lg:w-full flex items-center justify-center grayscale opacity-40 hover:grayscale-0 hover:opacity-100 transition-all duration-500">
                                <img
                                    src={dev.logo}
                                    alt={dev.name}
                                    className={`${dev.name === 'Meraas' ? 'h-10 md:h-12 lg:h-14' :
                                        ...
                                        } w-auto max-w-[140px] md:max-w-[250px] lg:max-w-[300px] object-contain px-4`}
                                    title={dev.name}
                                />
                            </div>
                        ))}
